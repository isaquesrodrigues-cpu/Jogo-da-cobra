...
let cobra = []; //Armazenar as posições da cobra
let direcao= {x: 1, y: 0} //Direção da próxima tickada

let comida = {x: 0,y:0} //Armazenar a posição da comida
let pontuacao = 0;
...
function criarGrade () {
    ...
    for (let y; y < LINHAS; y++) {
        const linha = [];

        for ( let x = 0; x< COLUNAS;x++){
            const celula = Document.createElemente('div');
